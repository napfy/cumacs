...
(defun cumacs-golang-build ()
  (interactive)
  (compile (concat "go build -ldflags=\"-w -s\" -o "  (go-guess-gopath) "bin/" (file-name-sans-extension (buffer-name)) " -v " (buffer-file-name))))

(defun cumacs-golang-linux-build ()
  (interactive)
  (compile (concat "GOOS=linux GOARCH=amd64 go build -ldflags=\"-w -s\" -o "  (go-guess-gopath) "bin/" (file-name-sans-extension (buffer-name)) " -v " (buffer-file-name))))

(defun cumacs-golang-win-build ()
  (interactive)
  (compile (concat "GOOS=windows GOARCH=amd64 go build -ldflags=\"-w -s\" -o "  (go-guess-gopath) "bin/" (file-name-sans-extension (buffer-name)) " -v " (buffer-file-name))))
